...
public class MySqlInteractions
{
    ...
    /// <returns></returns>
    public List<string>[] Select(string query, uint numberOfColumns, MySqlParameter[] parameters = null)
    {
        //Create a list to store the result
        List<string>[] results = new List<string>[numberOfColumns];
        ...
        if (OpenConnection())
        {
            ...
            while (dataReader.Read())
            {
                for (int i = 0; i < numberOfColumns; i++)
                {
                    results[i].Add(dataReader.GetValue(i).ToString());
                }
            }
    ...
    /// <returns></returns>
    public async Task<int> Insert(string query, MySqlParameter[] parameters)
    {
        ...
        int results = await sqlCmd.ExecuteNonQueryAsync();
